Node_hub_score_centrality_plot <- function(eigenvector_centrality_all, labels_y, max_year,legend) {
  library(reshape2)
  library(dplyr)
  library(ggplot2)
  ...
  colnames(data_long) <- c("Provinces", "Time", "Hub_score")
  ...
  max_values <- data_long %>%
    group_by(Time) %>%
    slice(which.max(Hub_score)) %>%
    ungroup()
  
  # Create the plot
  p_eigen <- ggplot(data_long, aes(x = Time, y = Provinces, fill = Hub_score)) + 
    geom_tile(color = NA) +  
