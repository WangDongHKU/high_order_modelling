# 定义函数
plot_page_rank_centrality_map <- function(g) {
  library(igraph)
  library(ggplot2)
  library(ggraph)
  library(maps)
  ...
  province_data <- data.frame(name = province,value = page.rank(g,directed = TRUE,weights =1/E(g)$weight)$vector)
  # 确保数据合并正确
  ...
  plot1 <- ggplot(data = china_map) +
    ...
    scale_fill_gradient2(low = "#4A7056", mid = "white",high = "#976666",midpoint = 100) +
    theme_void() +
    labs(fill = "Page rank")+
    theme(
