plot_cluster_Infection_network <- function(g, data_lst) {
library(igraph)
library(ggplot2)
library(ggraph)
library(maps)
...
library(rnaturalearthdata)
  ...
  merged_map_data <- rbind(china_province_map_data, taiwan_map_data)
   ...
   communities <- cluster_walktrap(g,weights = E(g)$weight,modularity = TRUE) 
 ...
 # communities <- cluster_infomap(g)
  ...
  province_main_community <- aggregate(community ~ province, data = province_communities, 
                                       FUN = function(x) as.numeric(names(sort(table(x)))))
  
  merged_map_data$community <- province_main_community$community[match(merged_map_data$name, province_main_community$province)]
    #merged_map_data$community <- factor(merged_map_data$community, levels = sort(unique(merged_map_data$community)))
  ...
  plot_combined <- ggraph(g, layout = layout_matrix) +
    ...
    theme_void() +
    theme(#trip.background = element_rect(fill = "#F5F5F5", color = NA),
      legend.position = "right",
      legend.text = element_text(size = 6),
      legend.title = element_text(size = 6)
    ) +
    guides(
      size = guide_legend(title= "Infection No", keywidth = unit(0.01, "cm"), keyheight = unit(0.1, "cm"), order = 1),
      edge_alpha = "none",
      edge_color = guide_legend(title= "Infection Move", keywidth = unit(0.01, "cm"), keyheight = unit(0.1, "cm"), order = 2),
      edge_width = "none",
      color = "none",
      fill = guide_legend(title= "Communities", keywidth = unit(0.4, "cm"), keyheight = unit(0.3, "cm"), order = 3)
    )
