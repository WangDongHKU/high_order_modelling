mid_peorid1 <-  function(dailyincidence, Mobility,p_report) {
  ...
  source("linearfitting_extend_mid.r")
  ...
  # 进行线性拟合
  p1 <- linearfitting_extend_mid(filtered_mobility, (1+filtered_dates)) + labs(
    y = "Cumulative reported infection"
  ) 
  
  source("linearfitting_extend_mid1.r")
  filtered_deff <- Deff[location,selected_elements]  
  # 进行线性拟合
  p2 <- linearfitting_extend_mid1(filtered_deff, 1+filtered_dates) + labs(
    y = "Cumulative reported infection"
  ) 
