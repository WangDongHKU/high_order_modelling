# 定义函数
plot_NPI_impact_map <- function(pred_ili_dfRt) {
  library(igraph)
  library(ggplot2)
  library(ggraph)
  library(maps)
  ...
  plot1 <- ggplot(data = china_map) +
    ...
    # scale_fill_gradient(high = "white", low = "#CB9B8F") +
    scale_fill_gradient2(low = "#4A7056", mid = "white",high = "#976666",midpoint = 0.6) +
  
    theme_void() +
    labs(fill = "NPI impact")+
    theme(
