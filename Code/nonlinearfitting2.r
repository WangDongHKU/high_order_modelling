nonlinearfitting2 <- function(Deff_no_na, date_index_no_na) {
  library(robustbase)
  library(minpack.lm)
  ...
  # Fit models for Ï„ = 0.1, 0.5, 0.9
  # First fit Ï„=0.2
  model_q10 <- nlrq(
    y ~ a * Deff +b,
    data = data,
    start = list(a = 0.26, b = 1),
    tau = 0.1
  )
