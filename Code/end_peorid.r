end_peorid <-  function(dailyincidence, Mobility,p_report) {
  ...
  # 进行线性拟合
  p1 <- linearfitting_end(1+filtered_mobility, 1+filtered_dates)
  
  source("linearfitting_end1.r")
  ...
  # 进行线性拟合
  p2 <- linearfitting_end1(filtered_deff, 1+filtered_dates)
  
  
  source("nonlinearfitting2.r")
  source("nonlinearfitting_end.r")
  ...
  p3 <- nonlinearfitting_end(Deff_no_na,date_index_no_na)
  library(magick)
  library(pdftools)
