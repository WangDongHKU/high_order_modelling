# 定义函数
plot_betweenness_centrality_map <- function(g) {
  library(igraph)
  library(ggplot2)
  library(ggraph)
  library(maps)
  ...
  province_data <- data.frame(name = province,value = betweenness(g,weights = 1/(1+E(g)$weight),normalized = TRUE,directed = TRUE))
  # 确保数据合并正确
