Node_Degree_centrality_plotin <- function(eigenvector_centrality_all, labels_y, max_year,legend) {
  library(reshape2)
  library(dplyr)
  library(ggplot2)
  ...
  p_eigen <- ggplot(data_long, aes(x = Time, y = Provinces, fill = sqrt(Degree + 1))) + 
    ...
    geom_point(data = max_values, aes(x = Time, y = Provinces), color = "#009E73", shape = 21, fill = "yellow", size = 0.5, stroke = 0.5) + 
    scale_x_continuous(breaks = seq(from = 0, to = max_year*365.25, by = 365.25), labels = seq(2020, 2020+max_year, 1)) +
    labs(y = "Provinces", fill = expression(sqrt("Degree(in)"))) + 
