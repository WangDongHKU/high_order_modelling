Node_Eigenvector_centrality_plot <- function(eigenvector_centrality_all, labels_y, max_year,legend) {
  library(reshape2)
  library(dplyr)
  library(ggplot2)
  ...
  colnames(data_long) <- c("Provinces", "Time", "Eigenvector")
  ...
  max_values <- data_long %>%
    group_by(Time) %>%
    slice(which.max(Eigenvector)) %>%
    ungroup()
  
  
  
  
  p_eigen <- ggplot(data_long, aes(x = Time, y = Provinces, fill = Eigenvector)) + 
    geom_tile(color = NA) +  
