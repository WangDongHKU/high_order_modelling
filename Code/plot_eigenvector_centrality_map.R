# 定义函数
plot_eigenvector_centrality_map <- function(g) {
  library(igraph)
  library(ggplot2)
  library(ggraph)
  library(maps)
  ...
  province_data <- data.frame(name = province,value = eigen_centrality(g, weights = E(g)$weight,directed = TRUE,scale = TRUE)$vector)
  # 确保数据合并正确
